...
public class bookSlotController {
    ...
    public String getSlots(Model model, @RequestParam(name = "date", defaultValue = "") String date, HttpSession session){
        ...
        for (Slot slot : slots){
            ...
            if (!bookSlots.isEmpty()){
                ...
                BookSlot bookSlot = bookSlotRepository.findBySlotCodeAndGameDateAndConfirmStatus(slot.getSlotCode(), dateModified, "accepted");
                if (bookSlot != null){
                    statusColor = "red";
                    status = "Already Booked by another user";
                }
    ...
    public String myRequests(Model model, HttpSession session){
        Map user = (Map) session.getAttribute("user");
        List<BookSlot> bookSlots = bookSlotRepository.findByBookedByOrderBySlotCodeDescGameDateDescCourtCodeDesc(user.get("mobileNo").toString());
